...
const markPaidFromWebhook = async (
  ...
  order.providerSessionId =
    normalized.providerSessionId ?? order.providerSessionId;
  order.status = "paid";
  await order.save();
  ...
  if (order.source === "ecommerce" && order.ecommerce?.items?.length) {
    ...
    for (const item of order.ecommerce.items) {
      ...
      if (item.variantId && Array.isArray(prod.variants)) {
        ...
        if (idx >= 0) {
          prod.variants[idx].stock = Math.max(
            0,
            (prod.variants[idx].stock || 0) - item.qty
          );
        }
      } else {
  ...
  if (order.itemType === "course" && order.course) {
    ...
    await EnrollmentServices.enrollSelf(
      String(order.course),
      normalized.userId
    );
